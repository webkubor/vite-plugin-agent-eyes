---
name: agent-eyes
description: "给 AI agent 的自愈遥测层 — 在 Vite 项目里读结构化运行时日志（API/错误/控制台/代理 header）+ 脱敏登录态画像 + 截图 + DOM 快照 + 提交前 guard 报告，自我诊断、修复、验证，无需人读代码"
version: 0.9.0
---
...
- 需要还原已登录 UI 或浏览器控制时，先看 `log/<port>/auth-state.json` 确认当前账号画像。
