---
name: agent-eyes
description: "给 AI agent 的自愈遥测层 — 在 Vite 项目里读结构化运行时日志（API/错误/控制台/代理 header）+ 截图 + DOM 快照，自我诊断、修复、验证，无需人读代码"
version: 0.3.0
---
...
3. **按线索下钻**：
   ...
   - 控制台警告/调试信息 → `log/console.log` 看全级别输出（React dev warning、库 deprecation 等）。
   - 登录/cookie/CORS/302 等网络层 → `log/proxy-<host>.log` 看 `Cookie(req)` 与 `Set-Cookie`。
   - 视觉问题 → `log/snapshots/` 看错误截图（PNG）+ DOM 快照（HTML）。
...
| 文件 | 看什么 |
|------|--------|
| `log/errors.log` | API 失败 + 前端运行时错误（聚合去重 + 频率） |
| `log/console.log` | 全级别控制台输出（log/warn/error/info/debug） |
| `log/api-calls.log` | API 全量 + 路由，带请求/响应体 |
| `log/proxy-<host>.log` | 代理层 Cookie / Set-Cookie 属性 / status（多代理按 host 分文件） |
| `log/snapshots/err-*.png` | 错误截图（需 CDP，自动检测端口） |
| `log/snapshots/dom-*.html` | DOM 快照（始终可用，无需 CDP） |
